...
class SegmentationModule(SegmentationModuleBase):
    ...
    def forward(self, feed_dict, epoch, *, segSize=None):
        ...
        if segSize is None:
            ...
            loss = self.crit(p, feed_dict[1])
            acc = self.pixel_acc(torch.round(nn.functional.softmax(p[0], dim=1)).long(), feed_dict[1][0].cuda())
        ...
        #inference
            ...
            p = self.unet(feed_dict[0])
            loss = self.crit((p[0], p[1]), (feed_dict[1][0].long().cuda(), feed_dict[1][1]))
