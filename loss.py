...
class DualLoss(nn.Module):
    def __init__(self, num_classes=3, lmbda=10, epsilon=10e-6, mode="train"):
        super(DualLoss, self).__init__()
        self.epsilon = epsilon
        self.lmbda = lmbda
        self.channels = num_classes 
        self.CELoss = nn.CrossEntropyLoss(weight=torch.tensor([1.0, 1.0, 1.0, 1.0])).cuda()
    ...
    def forward(self, pred, target, epoch=0):
        ...
        seg, edge_in = pred
        seg_t, edge_t = target
        seg_t = seg_t.long().cuda()

        ce = self.CELoss(seg, seg_t)
        dice = dice_loss(seg_t, seg)
        #edge
        ...
        '''
        #positives
        soft_seg = torch.softmax(seg, dim=1)
        valid = (seg_t==3).float() * soft_seg[:,3,]
        valid = valid.cuda()
        valid_sum = torch.sum(torch.sum(valid, dim=1), dim=1)
        true_scar = torch.sum(torch.sum((seg_t==3).float(), dim=1), dim=1)
        mse = self.mseloss(valid_sum, true_scar)
 
        #false postives
        inv_valid = soft_seg[:,3,]*(torch.ones(valid.shape).cuda() - valid)
        inv_valid = torch.sum(torch.sum(inv_valid, dim=1), dim=1)
        #fp_loss = self.mseloss(inv_valid, torch.zeros(inv_valid.shape).cuda())
        '''

        return dice + ce + edge #+ 0.1*att + dice + 
