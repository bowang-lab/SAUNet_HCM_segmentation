...
class AdjustBrightness(object):
    ...
    def adjust_brightness(self, img, bf):
        if isinstance(img, Image.Image):
            img = np.array(img)
        ...
        img = np.clip(img*bf, 0.0, 1.0)
        img = img * 255
        return Image.fromarray(img.astype(np.uint32), mode="I")
...
class AdjustContrast(object):
    ...
    def adjust_contrast(self, img, cf):
        if isinstance(img, Image.Image):
            img = np.array(img)
        ...
        img = np.clip(0.5+cf*(img-0.5), 0.0, 1.0)
        img = img * 255
        return Image.fromarray(img.astype(np.uint32), mode="I")
...
class PaddingCenterCropTest(object):
    def __init__(self, size):
        '''
        Argument crop_size is an integer for square cropping only.
    ...
    def __call__(self, img):
        ...
        tw, th = self.size
        if w >= tw and h >= th:  # crop a center patch
            x1 = int(self.round_num((w - tw) / 2.))
            y1 = int(self.round_num((h - th) / 2.))
            return img.crop((x1, y1, x1 + tw, y1 + th))
        else:  # pad zeros and do center crop
            pad_h = max(th - h, 0)
            pad_w = max(tw - w, 0)
            ...
            # do center crop
            x1 = max(int(self.round_num((w - tw) / 2.)), 0)
            y1 = max(int(self.round_num((h - th) / 2.)), 0)
            return img.crop((x1, y1, x1 + tw, y1 + th))
    
    def round_num(self, x):
...
class Scale(object):
    def __init__(self, size=512, random_scale=False):
        self.size = size
    ...
    def __call__(self, img, mask):
        ...
        if self.random_scale:
            new_w = int(random.uniform(0.5, 2) * w)
            new_h = int(random.uniform(0.5, 2) * h)
            return (
...
# Code below doesn't really work yet.
class Sharpness(object): 
    def __call__(self, img, mask):
        ...
        img = (np.array(img)[:,:,0]/255.0).astype(np.int32)*(max_val-min_val)+min_val
        print(img.max(), max_val)
